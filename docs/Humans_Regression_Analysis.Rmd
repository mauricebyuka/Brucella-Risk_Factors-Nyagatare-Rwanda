...
library(knitr)             # For a better formatting of tabular output
```
...
Despite a VIF of 14.07 for the OccupGroup variable, it was retained in the model due to its importance in the overall predictive performance, as verified by stepwise regression based on the Akaike Information Criterion (AIC). Stepwise model selection (see below) was conducted to reduce the number of predictors and identify the most parsimonious model.
...
plot(roc_curve, main = "ROC Curve for Human Brucellosis Prediction Model", col = "blue")
...
####  Step 6: Write model results to table
...
tidy_model <- calculate_results(step_model)
...
tidy_model <- tidy_model %>%
  mutate(term = case_when(
    ...
    str_detect(term, "contact_with_waste") ~ str_replace(term, "contact_with_waste", "Contact with waste: "),
    str_detect(term, "aborted_fetus") ~ str_replace(term, "aborted_fetus", "Handled aborted fetus: "),
    str_detect(term, "milking") ~ str_replace(term, "milking", "Milking: "),
    str_detect(term, "Herded_livestock") ~ str_replace(term, "Herded_livestock", "Herded livestock: ")
  )) %>%drop_na()

# Write to table
out_table <- tidy_model %>% select(c("term", "odds_ratio", "lower_ci", "upper_ci", "p.value"))
write.csv(out_table, "../output/Humans_multivariable_results_with_ci.csv", row.names = FALSE)
