...
```{r}
data <- read.csv('Humans_regression_data.csv') # Output of the previous step
```
...
predictors <- c("Gender", "Age", "Marital_status", "Education", "OccupGroup", 
...
# write.csv(univariable_results, "humans_univariable_logistic.csv", row.names = FALSE)
```
...
####  Step 6: Plot the odds ratios for the significant risk factors
This was not reported as I believe that the tabular format is a better presentation of these data is it shows the reference categories and exact vlues of the parameters.
...
significant_terms %>%
  ...
  coord_flip() +
  labs(title = "Significant Risk Factors for Brucellosis (Odds Ratios)", 
       x = "Risk Factor", 
